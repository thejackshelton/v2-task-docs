...
Users type in inputs, data is fetched from different sources, timers fire, and state needs to be updated. Tasks help you manage all of these changes by:
...
<details class="mt-6 bg-neutral-900 p-[1ch] rounded-md">
  ...
  Key differences:
  * Tasks can run on both server and browser
  * Tasks run before rendering (unlike effects which run after)
  * Tasks can execute both async and sync code
...
Your app can run entirely in the browser (CSR) or start on a server (SSR/SSG) - this choice affects where your code runs at page load.

### How can I know where my code is running?

#### Checking where code runs

- Browser: Look in your browser's DevTools console
- Server: Look in your terminal where you ran `npm run dev` or `npm run preview`

> Tip: Try adding `console.log("Hello!")` to your task - where you see the message tells you where it ran.
